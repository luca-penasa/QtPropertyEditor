...
static MetaTypeRegistration<QtPushButtonActionWrapper>
    thisInstantiationRegistersQtPushButtonActionWrapperWithQt;

QList<QByteArray> getPropertyNames(QObject *object) {
  QList<QByteArray> propertyNames = getMetaPropertyNames(*object->metaObject());
  foreach (const QByteArray &dynamicPropertyName,
           object->dynamicPropertyNames()) {
...
QList<QByteArray> getNoninheritedPropertyNames(QObject *object) {
  ...
  QList<QByteArray> superPropertyNames =
      getMetaPropertyNames(*object->metaObject()->superClass());
...
QObject *descendant(QObject *object, const QByteArray &pathToDescendantObject) {
  // Get descendent object specified by "path.to.descendant", where "path", "to"
  // and "descendant" are the object names of objects with the parent->child
  // relationship object->path->to->descendant.
  if (!object || pathToDescendantObject.isEmpty()) return 0;
  if (pathToDescendantObject.contains('.')) {
    ...
    foreach (QByteArray name, descendantObjectNames) {
      ...
      if (!object) return 0;  // Invalid path to descendant object.
    }
    return object;
  }
...
QSize getTableSize(const QTableView *table) {
  ...
  for (int i = 0; i < table->model()->rowCount(); i++) h += table->rowHeight(i);
  return QSize(w, h);
}
...
void QtAbstractPropertyModel::setProperties(const QString &str) {
  ...
  foreach (const QString &field, fields) {
    if (!field.trimmed().isEmpty()) addProperty(field);
  }
}
...
const QMetaProperty QtAbstractPropertyModel::metaPropertyAtIndex(
    const QModelIndex &index) const {
  QObject *object = objectAtIndex(index);
  if (!object) return QMetaProperty();
  QByteArray propertyName = propertyNameAtIndex(index);
  if (propertyName.isEmpty()) return QMetaProperty();
  // Return metaObject with same name.
  const QMetaObject *metaObject = object->metaObject();
  int numProperties = metaObject->propertyCount();
  for (int i = 0; i < numProperties; ++i) {
    const QMetaProperty metaProperty = metaObject->property(i);
    if (QByteArray(metaProperty.name()) == propertyName) return metaProperty;
  }
  return QMetaProperty();
}

QVariant QtAbstractPropertyModel::data(const QModelIndex &index,
                                       int role) const {
  ...
  if (role == Qt::DisplayRole || role == Qt::EditRole) {
    ...
    QByteArray propertyName = propertyNameAtIndex(index);
    if (propertyName.isEmpty()) return QVariant();
    return object->property(propertyName.constData());
  }
  return QVariant();
}

bool QtAbstractPropertyModel::setData(const QModelIndex &index,
...
Qt::ItemFlags QtAbstractPropertyModel::flags(const QModelIndex &index) const {
  ...
  if (metaProperty.isWritable() ||
      object->dynamicPropertyNames().contains(propertyName))
    flags |= Qt::ItemIsEditable;
  return flags;
}

void QtPropertyTreeModel::Node::setObject(
    QObject *object, int maxChildDepth,
  ...
  // Compiled properties (but exclude objectName as this is displayed for the
  // object node itself).
  const QMetaObject *metaObject = object->metaObject();
  int numProperties = metaObject->propertyCount();
  for (int i = 0; i < numProperties; ++i) {
  ...
  if (maxChildDepth > 0 || maxChildDepth == -1) {
    if (maxChildDepth > 0) --maxChildDepth;
...
QtPropertyTreeModel::Node *QtPropertyTreeModel::nodeAtIndex(
    const QModelIndex &index) const {
  try {
...
QModelIndex QtPropertyTreeModel::index(int row, int column,
                                       const QModelIndex &parent) const {
  // Return a model index whose internal pointer references the appropriate tree
  // node.
...
QModelIndex QtPropertyTreeModel::parent(const QModelIndex &index) const {
  ...
  if (!parentNode || parentNode == &_root) return QModelIndex();
  int row = 0;
  Node *grandparentNode = parentNode->parent;
  if (grandparentNode) row = grandparentNode->children.indexOf(parentNode);
...
bool QtPropertyTreeModel::setData(const QModelIndex &index,
                                  const QVariant &value, int role) {
  if (!index.isValid()) return false;
  if (role == Qt::EditRole) {
    QObject *object = objectAtIndex(index);
...
QVariant QtPropertyTreeModel::headerData(int section,
...
QObject *QtPropertyTableModel::objectAtIndex(const QModelIndex &index) const {
  if (_objects.size() <= index.row()) return 0;
  QObject *object = _objects.at(index.row());
  // If property names are specified, check if name at column is a path to a
...
QByteArray QtPropertyTableModel::propertyNameAtIndex(
    const QModelIndex &index) const {
  ...
  // If property names are NOT specified, return the metaObject's property name
  // at column.
  QObject *object = objectAtIndex(index);
  if (!object) return QByteArray();
  const QMetaObject *metaObject = object->metaObject();
  int numProperties = metaObject->propertyCount();
  if (numProperties > index.column())
    return QByteArray(metaObject->property(index.column()).name());
  // If column is greater than the number of metaObject properties, check for
  // dynamic properties.
...
QModelIndex QtPropertyTableModel::index(
    int row, int column, const QModelIndex & /* parent */) const {
  return createIndex(row, column);
}

QModelIndex QtPropertyTableModel::parent(
    const QModelIndex & /* index */) const {
  return QModelIndex();
}
...
int QtPropertyTableModel::columnCount(const QModelIndex & /* parent */) const {
  ...
  if (!propertyNames.isEmpty()) return propertyNames.size();
...
QVariant QtPropertyTableModel::headerData(int section,
                                          Qt::Orientation orientation,
                                          int role) const {
  if (role == Qt::DisplayRole) {
...
bool QtPropertyTableModel::insertRows(int row, int count,
                                      const QModelIndex &parent) {
  // Only valid if we have an object creator method.
  if (!_objectCreator) return false;
  bool columnCountWillAlsoChange =
      _objects.isEmpty() && propertyNames.isEmpty();
...
bool QtPropertyTableModel::removeRows(int row, int count,
                                      const QModelIndex &parent) {
  beginRemoveRows(parent, row, row + count - 1);
  for (int i = row; i < row + count; ++i) delete _objects.at(i);
...
bool QtPropertyTableModel::moveRows(const QModelIndex & /*sourceParent*/,
  ...
  endResetModel();
  reorderChildObjectsToMatchRowOrder(
      sourceRow <= destinationRow ? sourceRow : destinationRow);
...
QWidget *QtPropertyDelegate::createEditor(QWidget *parent,
  ...
  if (value.isValid()) {
    if (value.type() == QVariant::Bool) {
      ...
      // We don't need to do anything special for an integer, we'll just use the
      // default QSpinBox. However, we do need to check if it is an enum. If so,
      // we'll use a QComboBox editor.
      const QtAbstractPropertyModel *propertyModel =
          qobject_cast<const QtAbstractPropertyModel *>(index.model());
      if (propertyModel) {
        const QMetaProperty metaProperty =
            propertyModel->metaPropertyAtIndex(index);
        if (metaProperty.isValid() && metaProperty.isEnumType()) {
          ...
          if (numKeys > 0) {
            ...
            QByteArray currentKey =
                QByteArray(metaEnum.valueToKey(value.toInt()));
      ...
      // Return a QLineEdit. Parsing will be done in displayText() and
      // setEditorData().
      QLineEdit *editor = new QLineEdit(parent);
...
void QtPropertyDelegate::setEditorData(QWidget *editor,
                                       const QModelIndex &index) const {
  QStyledItemDelegate::setEditorData(editor, index);
}

void QtPropertyDelegate::setModelData(QWidget *editor,
                                      QAbstractItemModel *model,
                                      const QModelIndex &index) const {
  QVariant value = index.data(Qt::DisplayRole);
  if (value.isValid()) {
    if (value.type() == QVariant::Double) {
      // Set model's double value data to numeric representation in QLineEdit
      // editor. Conversion from text to number handled by QVariant.
      QLineEdit *lineEditor = qobject_cast<QLineEdit *>(editor);
      if (lineEditor) {
        QVariant value = QVariant(lineEditor->text());
        bool ok;
        double dval = value.toDouble(&ok);
        if (ok) model->setData(index, QVariant(dval), Qt::EditRole);
        return;
      }
    } else if (value.type() == QVariant::Int) {
      // We don't need to do anything special for an integer.
      // However, if it's an enum we'll set the data based on the QComboBox
      // editor.
      QComboBox *comboBoxEditor = qobject_cast<QComboBox *>(editor);
      if (comboBoxEditor) {
        QString selectedKey = comboBoxEditor->currentText();
        const QtAbstractPropertyModel *propertyModel =
            qobject_cast<const QtAbstractPropertyModel *>(model);
        if (propertyModel) {
          const QMetaProperty metaProperty =
              propertyModel->metaPropertyAtIndex(index);
          if (metaProperty.isValid() && metaProperty.isEnumType()) {
            ...
            bool ok;
            int selectedValue =
                metaEnum.keyToValue(selectedKey.toLatin1().constData(), &ok);
            if (ok)
              model->setData(index, QVariant(selectedValue), Qt::EditRole);
            return;
          }
        }
        // If we got here, we have a QComboBox editor but the property at index
        // is not an enum.
      }
    } else if (value.type() == QVariant::Size) {
      QLineEdit *lineEditor = qobject_cast<QLineEdit *>(editor);
      if (lineEditor) {
        ...
        QRegularExpressionMatch match =
            regex.match(lineEditor->text().trimmed());
      ...
      //            QLineEdit *lineEditor = qobject_cast<QLineEdit*>(editor);
      //            if(lineEditor) {
      //                // Parse formats: (r,g,b) or (r g b) or (r,g,b,a) or (r
      //                g b a) <== () are optional QRegularExpression
      //                regex("\\s*\\(?"
      //                                         "\\s*(\\d+)\\s*"
      //                                         "[,\\s]\\s*(\\d+)\\s*"
      //                                         "[,\\s]\\s*(\\d+)\\s*"
      ...
      //                QRegularExpressionMatch match =
      //                regex.match(lineEditor->text().trimmed());
      //                if(match.hasMatch() && (match.capturedTexts().size() ==
      //                4 || match.capturedTexts().size() == 5)) {
      ...
      //                        if(rok && gok && bok && aok)
      //                            model->setData(index, QColor(r, g, b, a),
      //                            Qt::EditRole);
      //                    }
      //                }
      //            }
...
QString QtPropertyDelegate::displayText(const QVariant &value,
                                        const QLocale &locale) const {
  if (value.isValid()) {
    if (value.type() == QVariant::Size) {
      // w x h
      QSize size = value.toSize();
      return QString::number(size.width()) + QString(" x ") +
      ...
      return QString("(") + QString::number(point.x()) + QString(", ") +
             QString::number(point.y()) + QString(")");
    } else if (value.type() == QVariant::PointF) {
      // (x, y)
      ...
      return QString("[(") + QString::number(rect.x()) + QString(", ") +
             QString::number(rect.y()) + QString("), ") +
             QString::number(rect.width()) + QString(" x ") +
...
void QtPropertyDelegate::paint(QPainter *painter,
                               const QStyleOptionViewItem &option,
                               const QModelIndex &index) const {
  QVariant value = index.data(Qt::DisplayRole);
  if (value.isValid()) {
    if (value.type() == QVariant::Bool) {
      ...
      buttonOption.rect = QStyle::alignedRect(
          option.direction, Qt::AlignLeft, checkBoxRect.size(),
      ...
      // We don't need to do anything special for an integer.
      // However, if it's an enum want to render the key name instead of the
      // value. This cannot be done in displayText() because we need the model
      // index to get the key name.
      const QtAbstractPropertyModel *propertyModel =
          qobject_cast<const QtAbstractPropertyModel *>(index.model());
      if (propertyModel) {
        const QMetaProperty metaProperty =
      ...
      if (value.canConvert<QtPushButtonActionWrapper>()) {
        ...
        // buttonOption.rect = QRect(option.rect.x() + 5, option.rect.y() + 5,
...
bool QtPropertyDelegate::editorEvent(QEvent *event, QAbstractItemModel *model,
                                     const QStyleOptionViewItem &option,
  ...
  if (value.isValid()) {
    if (value.type() == QVariant::Bool) {
      ...
      // &buttonOption); // Only used to get size of native checkbox widget.
      // buttonOption.rect = QStyle::alignedRect(option.direction,
      // Qt::AlignLeft, checkBoxRect.size(), option.rect); // Our checkbox rect.
      // option.rect ==> cell
      // buttonOption.rect ==> check box
      // Here, we choose to allow clicks anywhere in the cell to toggle the
      // checkbox.
      if (!option.rect.contains(mouseEvent->pos())) return false;
      bool checked = value.toBool();
      QVariant newValue(!checked);  // Toggle model's bool value.
      bool success = model->setData(index, newValue, Qt::EditRole);
      // Update entire table row just in case some other cell also refers to the
      // same bool value. Otherwise, that other cell will not reflect the
      // current state of the bool set via this cell.
      if (success)
        model->dataChanged(index.sibling(index.row(), 0),
                           index.sibling(index.row(), model->columnCount()));
      return success;
    } else if (value.type() == QVariant::UserType) {
      if (value.canConvert<QtPushButtonActionWrapper>()) {
        QMouseEvent *mouseEvent = static_cast<QMouseEvent *>(event);
...
QtPropertyTreeEditor::QtPropertyTreeEditor(QWidget *parent)
    : QTreeView(parent) {
...
QtPropertyTableEditor::QtPropertyTableEditor(QWidget *parent)
    : QTableView(parent) {
  setItemDelegate(&_delegate);
  setAlternatingRowColors(true);
  setModel(&tableModel);
  ...
  connect(verticalHeader(), SIGNAL(sectionMoved(int, int, int)), this,
          SLOT(handleSectionMove(int, int, int)));

  // Header context menus.
  horizontalHeader()->setContextMenuPolicy(Qt::CustomContextMenu);
  verticalHeader()->setContextMenuPolicy(Qt::CustomContextMenu);
  connect(horizontalHeader(), SIGNAL(customContextMenuRequested(QPoint)), this,
          SLOT(horizontalHeaderContextMenu(QPoint)));
  connect(verticalHeader(), SIGNAL(customContextMenuRequested(QPoint)), this,
          SLOT(verticalHeaderContextMenu(QPoint)));
...
void QtPropertyTableEditor::setIsDynamic(bool b) {
  ...
  if (QAbstractButton *cornerButton = findChild<QAbstractButton *>()) {
    ...
    // adjust the width of the vertical header to match the preferred corner
    // button width (unfortunately QAbstractButton doesn't implement any size
    // hinting functionality)
    QStyleOptionHeader opt;
    opt.text = cornerButton->text();
    // opt.icon = cornerButton->icon();
    QSize s = (cornerButton->style()
                   ->sizeFromContents(QStyle::CT_HeaderSection, &opt, QSize(),
                                      cornerButton)
...
void QtPropertyTableEditor::horizontalHeaderContextMenu(QPoint pos) {
  ...
  menu->addAction("Resize Columns To Contents", this,
...
void QtPropertyTableEditor::verticalHeaderContextMenu(QPoint pos) {
  ...
  QMenu *menu = new QMenu;
  if (_isDynamic) {
    QtPropertyTableModel *propertyTableModel =
        qobject_cast<QtPropertyTableModel *>(model());
...
void QtPropertyTableEditor::appendRow() {
  ...
  QtPropertyTableModel *propertyTableModel =
      qobject_cast<QtPropertyTableModel *>(model());
  if (!propertyTableModel || !propertyTableModel->objectCreator()) return;
...
void QtPropertyTableEditor::insertSelectedRows() {
  ...
  QList<int> rows;
  foreach (const QModelIndex &index, indexes) { rows.append(index.row()); }
  qSort(rows);
...
void QtPropertyTableEditor::handleSectionMove(int /* logicalIndex */,
                                              int oldVisualIndex,
                                              int newVisualIndex) {
  if (!_isDynamic) return;
  QtPropertyTableModel *propertyTableModel =
      qobject_cast<QtPropertyTableModel *>(model());
  if (!propertyTableModel) return;
  // Move objects in the model, and then move the sections back to maintain
  // logicalIndex order.
  propertyTableModel->moveRows(QModelIndex(), oldVisualIndex, 1, QModelIndex(),
                               newVisualIndex);
  disconnect(verticalHeader(), SIGNAL(sectionMoved(int, int, int)), this,
             SLOT(handleSectionMove(int, int, int)));
...
void QtPropertyTableEditor::keyPressEvent(QKeyEvent *event) {
  switch (event->key()) {
    ...
    case Qt::Key_Delete:
      if (_isDynamic &&
          QMessageBox::question(this, "Delete Rows?", "Delete selected rows?",
...
bool QtPropertyTableEditor::eventFilter(QObject *o, QEvent *e) {
  if (e->type() == QEvent::Paint) {
    if (QAbstractButton *btn = qobject_cast<QAbstractButton *>(o)) {
      ...
      if (btn->isEnabled()) styleState |= QStyle::State_Enabled;
      if (btn->isActiveWindow()) styleState |= QStyle::State_Active;
      if (btn->isDown()) styleState |= QStyle::State_Sunken;
      opt.state = styleState;
      opt.rect = btn->rect();
      opt.text = btn->text();  // this line is the only difference to
                               // QTableCornerButton
      // opt.icon = btn->icon(); // this line is the only difference to
      // QTableCornerButton
